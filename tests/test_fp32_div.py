"""
FP32 除法器测试 - 验证 SpikeFP32Divider 的 bit-exactness
==========================================================
...
def test_basic_division():
    """测试基本除法"""
    from atomic_ops import SpikeFP32Divider
    
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    divider = SpikeFP32Divider().to(device)
    ...
    for a, b, expected in test_cases:
        ...
        divider.reset()
        result_pulse = divider(a_pulse, b_pulse)
        ...
        if np.isclose(result_float, pytorch_result, rtol=1e-5) or result_float == pytorch_result:
            passed += 1
            print(f"  ✓ {a} / {b} = {result_float} (期望 {pytorch_result})")
        else:
            print(f"  ✗ {a} / {b} = {result_float} (期望 {pytorch_result})")
    
    print(f"\n基本除法: {passed}/{len(test_cases)} 通过")
...
def test_random_ulp():
    ...
    return exact_match >= n_samples * 0.8  # 80% 精确匹配
