...
使用标准 PulseFloatingPointDecoder 解码
"""
import sys
sys.path.insert(0, "/home/dgxspark/Desktop/HumanBrain")
import torch
from SNNTorch.atomic_ops import PulseFloatingPointEncoder, SpikeFP8Adder_Spatial
from SNNTorch.atomic_ops.pulse_decoder import PulseFloatingPointDecoder


def test_fp8_adder_basic():
    """测试基本加法正确性（端到端浮点验证）"""
    print("=== FP8 加法器基本测试（端到端浮点验证）===")
...
def test_fp8_adder_zero():
    ...
    for a_val, b_val in test_cases:
        ...
        status = "✓" if match else "✗"
        print(f"  {a_fp8.float().item()} + {b_fp8.float().item()} = {snn_result:.4f} (期望:{expected:.4f}) {status}")
    
    return all_pass


def test_fp8_adder_random():
    """随机测试（端到端浮点验证）"""
    print("\n=== FP8 加法器随机测试（1000对）===")
    device = "cuda" if torch.cuda.is_available() else "cpu"
...
def test_fp8_adder_cancellation():
    """测试抵消场景"""
    print("\n=== FP8 加法器抵消测试 ===")
    device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    test_vals = [0.5, 1.0, 2.0, 4.0, 8.0]
    all_pass = True
    
    for val in test_vals:
        a_fp8 = torch.tensor([val], device=device).to(torch.float8_e4m3fn)
        b_fp8 = torch.tensor([-val], device=device).to(torch.float8_e4m3fn)
        expected = (a_fp8.float() + b_fp8.float()).to(torch.float8_e4m3fn).float().item()
        ...
        match = abs(snn_result - expected) < 1e-6
        if not match:
            all_pass = False
...
if __name__ == "__main__":
    basic_pass = test_fp8_adder_basic()
    zero_pass = test_fp8_adder_zero()
    random_rate = test_fp8_adder_random()
    cancel_pass = test_fp8_adder_cancellation()
    
    print("\n=== 总结 ===")
    print(f"基本测试: {'✓' if basic_pass else '✗'}")
    print(f"零值测试: {'✓' if zero_pass else '✗'}")
    print(f"随机测试: {random_rate:.1f}%")
    print(f"抵消测试: {'✓' if cancel_pass else '✗'}")
