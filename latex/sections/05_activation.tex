...
The exponential function $e^x$ is fundamental to many activations. We implement it using the Taylor series expansion given by Equation~\ref{eq:exp_taylor}:

\begin{equation}
e^x = \sum_{n=0}^{N} \frac{x^n}{n!} = 1 + x + \frac{x^2}{2!} + \frac{x^3}{3!} + \cdots
\label{eq:exp_taylor}
\end{equation}
where $x$ denotes the input value, $N$ denotes the number of terms, and $n!$ denotes the factorial of $n$.

\paragraph{Range Reduction}
For numerical stability and convergence, we first reduce the input range. Using $e^x = 2^{x/\ln 2}$, the range reduction formula is given by Equation~\ref{eq:range_reduce}:

\begin{equation}
e^x = 2^k \times e^r
\label{eq:range_reduce}
\end{equation}
where $k = \lfloor x / \ln 2 \rfloor$ denotes the integer part and $r = x - k \ln 2$ denotes the remainder, ensuring $|r| < \ln 2 / 2$.

\paragraph{Polynomial Approximation}
For the reduced argument $r$, we use a truncated Taylor series. The number of terms $N$ depends on the required precision:

\begin{itemize}
    \item FP32: $N = 12$ terms for $< 1$ ULP error
    \item FP64: $N = 18$ terms for $< 1$ ULP error
\end{itemize}

\paragraph{SNN Implementation}
Each Taylor term $x^n/n!$ is computed via repeated SNN multiplication (powers) with precomputed $1/n!$ constants, then accumulated using an SNN adder tree. The final result is scaled by $2^k$ via exponent adjustment.
