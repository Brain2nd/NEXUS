...
This work challenges the prevailing assumption that SNNs are inherently approximate. Conventional SNN approaches encode information in spike rates or timing, inherently limiting precision to statistical estimates. In contrast, we adopt a fundamentally different paradigm: treating each spike as a \textbf{binary digit} and constructing \textbf{digital logic gates} from spiking neurons. This approach transforms SNNs from analog computers into bit-exact digital circuits, achieving IEEE 754 compliance while retaining the event-driven efficiency of neuromorphic hardware.

We present \textbf{MofNeuroSim}, a framework that implements IEEE 754 floating-point arithmetic entirely through spiking neural networks using Generalized Leaky Integrate-and-Fire (GLIF) neurons. The framework achieves three critical objectives: (1) bit-exact precision matching traditional CPUs, (2) gradient-based trainability via Straight-Through Estimator, and (3) robustness under physical hardware constraints including noise and leakage. Our contributions are:
