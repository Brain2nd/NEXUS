...
The XOR gate cannot be implemented with a single neuron (it is not linearly separable). We implement it using a three-neuron network as given by Equation~\ref{eq:xor}:

\begin{equation}
\text{XOR}(x_1, x_2) = \text{OR}(x_1, x_2) - \text{AND}(x_1, x_2)
\label{eq:xor}
\end{equation}
where $x_1, x_2 \in \{0, 1\}$ denote the binary inputs. Equivalently, using the algebraic identity given by Equation~\ref{eq:xor_alt}:
...
\begin{equation}
\text{MUX}(a, b, s) = \text{OR}(\text{AND}(a, s), \text{AND}(b, \text{NOT}(s)))
\label{eq:mux}
\end{equation}
where $a, b \in \{0, 1\}$ denote the data inputs and $s \in \{0, 1\}$ denotes the select signal. When $s = 1$, the output is $a$; when $s = 0$, the output is $b$.
