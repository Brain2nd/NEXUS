...
class RippleCarryAdderNBit(nn.Module):
    ...
    def __init__(self, bits, neuron_template=None):
        ...
        # 预分配参数形状
        max_shape = (bits,)
        self.vec_adder = VecAdder(bits, neuron_template=neuron_template, max_param_shape=max_shape)
...
class BarrelShifterRight28(nn.Module):
    ...
    def __init__(self, neuron_template=None):
        ...
        # 预分配参数形状
        max_shape = (self.data_bits,)

        # 单个 VecMUX 处理所有层 (预分配参数)
...
class BarrelShifterRight28WithSticky(nn.Module):
    ...
    def __init__(self, neuron_template=None):
        ...
        # Sticky 计算：预分配参数
        self.sticky_or_tree = VecORTree(neuron_template=nt, max_param_shape=max_shape_16)
...
class FP8ToFP32Converter(nn.Module):
    ...
    def __init__(self, neuron_template=None):
        ...
        # 预分配参数形状
        max_shape_23 = (23,)  # FP32 尾数
        max_shape_8 = (8,)    # FP32 指数
        max_shape_1 = (1,)
        ...
        # E=0时选择0指数 - 向量化
        self.vec_exp_mux = VecMUX(neuron_template=nt, max_param_shape=max_shape_8)
        ...
        # 纯SNN MUX门 - 向量化 (预分配最大形状)
        self.vec_mux = VecMUX(neuron_template=nt, max_param_shape=max_shape_23)
...
class FP32ToFP8Converter(nn.Module):
    ...
    def __init__(self, neuron_template=None):
        ...
        # 预分配参数形状
        max_shape_4 = (4,)   # FP8 指数
        max_shape_3 = (3,)   # FP8 尾数
        ...
        # 溢出/下溢/正常结果选择 - 向量化
        self.vec_overflow_mux_e = VecMUX(neuron_template=nt, max_param_shape=max_shape_4)   # 4位并行
        self.vec_overflow_mux_m = VecMUX(neuron_template=nt, max_param_shape=max_shape_3)   # 3位并行
        self.vec_subnorm_select_mux_e = VecMUX(neuron_template=nt, max_param_shape=max_shape_4)  # 4位并行
        self.vec_underflow_mux_e = VecMUX(neuron_template=nt, max_param_shape=max_shape_4)  # 4位并行
        self.vec_underflow_mux_m = VecMUX(neuron_template=nt, max_param_shape=max_shape_3)  # 3位并行

        # 舍入后指数选择 - 向量化
        self.vec_round_exp_mux = VecMUX(neuron_template=nt, max_param_shape=max_shape_4)    # 4位并行
        ...
        # Subnormal 尾数选择 MUX - 向量化
        self.vec_subnorm_mux_m = VecMUX(neuron_template=nt, max_param_shape=max_shape_3)  # 3位并行
    ...
    def forward(self, fp32_pulse):
        ...
        S = sticky_bits[..., 0:1]
        for i in range(1, 19):
            S = self.sticky_or(S, sticky_bits[..., i:i+1])
        ...
        not_carry_3 = not_carry.expand_as(fp8_mant)  # [..., 3]
        fp8_mant_final = self.vec_mant_clear_and(not_carry_3, fp8_mant)  # [..., 3]
        ...
        # 选择最终指数 - 向量化
        mant_carry_4 = mant_carry.expand_as(exp_after_round)  # [..., 4]
        final_exp_pre = self.vec_round_exp_mux(mant_carry_4, exp_after_round, fp8_exp)  # [..., 4]
        ...
        for i in range(19):
            sticky_120 = self.sticky_120_or(sticky_120, fp32_mant[..., 4+i:5+i])
        ...
        for i in range(20):
            sticky_119 = self.sticky_119_or(sticky_119, fp32_mant[..., 3+i:4+i])
        ...
        for i in range(21):
            sticky_118 = self.sticky_118_or(sticky_118, fp32_mant[..., 2+i:3+i])
        ...
        for i in range(22):
            mant_any_117 = self.mant_117_or(mant_any_117, fp32_mant[..., 1+i:2+i])
        ...
        # subnorm_m0
        subnorm_m0_t1 = self.mux_subnorm_m0(is_exp_117, sub_m0_117, zeros)  # 117 或 默认0
        subnorm_m0_t2 = self.mux_subnorm_m0(is_exp_118, sub_m0_118, subnorm_m0_t1)  # 118 或 上一步
        subnorm_m0_t3 = self.mux_subnorm_m0(is_exp_119, sub_m0_119, subnorm_m0_t2)  # 119 或 上一步
        subnorm_m0 = self.mux_subnorm_m0(is_exp_120, sub_m0_120, subnorm_m0_t3)  # 120 或 上一步
        # subnorm_m1
        subnorm_m1_t1 = self.mux_subnorm_m1(is_exp_117, sub_m1_117, zeros)
        subnorm_m1_t2 = self.mux_subnorm_m1(is_exp_118, sub_m1_118, subnorm_m1_t1)
        subnorm_m1_t3 = self.mux_subnorm_m1(is_exp_119, sub_m1_119, subnorm_m1_t2)
        subnorm_m1 = self.mux_subnorm_m1(is_exp_120, sub_m1_120, subnorm_m1_t3)
        # subnorm_m2
        subnorm_m2_t1 = self.mux_subnorm_m2(is_exp_117, sub_m2_117, zeros)
        subnorm_m2_t2 = self.mux_subnorm_m2(is_exp_118, sub_m2_118, subnorm_m2_t1)
        subnorm_m2_t3 = self.mux_subnorm_m2(is_exp_119, sub_m2_119, subnorm_m2_t2)
        subnorm_m2 = self.mux_subnorm_m2(is_exp_120, sub_m2_120, subnorm_m2_t3)
        ...
        # 选择最终结果 - 向量化（4位指数并行）
        is_overflow_4 = is_overflow.expand_as(nan_exp)  # [..., 4]
        e_ov = self.vec_overflow_mux_e(is_overflow_4, nan_exp, final_exp_pre)  # [..., 4]
        # Subnormal 时 E=0，但如果 subnormal 进位则 E=1
        subnorm_overflow_4 = subnorm_overflow.expand_as(subnorm_overflow_exp)  # [..., 4]
        e_subnormal_val = self.vec_subnorm_exp_mux(subnorm_overflow_4, subnorm_overflow_exp, zero_exp)  # [..., 4]
        is_subnormal_4 = is_subnormal.expand_as(e_subnormal_val)  # [..., 4]
        e_sub = self.vec_subnorm_select_mux_e(is_subnormal_4, e_subnormal_val, e_ov)  # [..., 4]
        is_underflow_4 = is_underflow.expand_as(zero_exp)  # [..., 4]
        final_exp = self.vec_underflow_mux_e(is_underflow_4, zero_exp, e_sub)  # [..., 4]

        # 选择最终结果 - 向量化（3位尾数并行）
        is_overflow_3 = is_overflow.expand_as(nan_mant)  # [..., 3]
        m_ov = self.vec_overflow_mux_m(is_overflow_3, nan_mant, fp8_mant_final)  # [..., 3]
        is_subnormal_3 = is_subnormal.expand_as(subnorm_mant)  # [..., 3]
        m_sub = self.vec_subnorm_mux_m(is_subnormal_3, subnorm_mant, m_ov)  # [..., 3]
        is_underflow_3 = is_underflow.expand_as(zero_mant)  # [..., 3]
        final_mant = self.vec_underflow_mux_m(is_underflow_3, zero_mant, m_sub)  # [..., 3]
    ...
    def reset(self):
        ...
        self.exp_inc.reset()
        # 向量化门电路
        self.vec_mant_clear_and.reset()
        self.vec_overflow_mux_e.reset()
        self.vec_overflow_mux_m.reset()
        self.vec_subnorm_select_mux_e.reset()
        self.vec_underflow_mux_e.reset()
        self.vec_underflow_mux_m.reset()
        self.vec_round_exp_mux.reset()
        self.vec_subnorm_exp_mux.reset()
        self.vec_subnorm_mux_m.reset()
        self.subnorm_cmp_low.reset()
        self.subnorm_cmp_high.reset()
        # 纯 SNN 门电路
        self.sticky_120_or.reset()
        self.sticky_119_or.reset()
        self.sticky_118_or.reset()
        self.mant_117_or.reset()
        ...
        self.mux_subnorm_m0.reset()
        self.mux_subnorm_m1.reset()
        self.mux_subnorm_m2.reset()
...
class FP32ToFP16Converter(nn.Module):
    ...
    def __init__(self, neuron_template=None):
        ...
        # 结果选择MUX - 向量化 (预分配参数)
        self.vec_overflow_mux = VecMUX(neuron_template=nt, max_param_shape=(16,))     # 16位
        self.vec_underflow_mux = VecMUX(neuron_template=nt, max_param_shape=(16,))    # 16位
        self.vec_nan_mux = VecMUX(neuron_template=nt, max_param_shape=(16,))          # 16位
        self.vec_inf_mux = VecMUX(neuron_template=nt, max_param_shape=(16,))          # 16位
        self.vec_round_exp_mux = VecMUX(neuron_template=nt, max_param_shape=(5,))     # 5位
        self.vec_mant_clear_and = VecAND(neuron_template=nt, max_param_shape=(10,))   # 10位
    ...
    def forward(self, fp32_pulse):
        ...
        for i in range(1, 8):
            e_all_one = self.nan_exp_and(e_all_one, fp32_exp[..., i:i+1])

        m_any_one = fp32_mant[..., 0:1]
        for i in range(1, 23):
            m_any_one = self.nan_mant_or(m_any_one, fp32_mant[..., i:i+1])
        ...
        S = fp32_mant[..., 11:12]
        for i in range(11):
            S = self.sticky_or(S, fp32_mant[..., 12+i:13+i] if 12+i < 23 else zeros)
        ...
        not_carry = self.not_carry(mant_carry)
        # 向量化: 尾数清零
        not_carry_10 = not_carry.expand_as(fp16_mant)  # [..., 10]
        fp16_mant_final = self.vec_mant_clear_and(not_carry_10, fp16_mant)  # [..., 10]
        ...
        # 向量化: 指数选择
        mant_carry_5 = mant_carry.expand_as(exp_after_round)  # [..., 5]
        final_exp = self.vec_round_exp_mux(mant_carry_5, exp_after_round, fp16_exp)  # [..., 5]
        ...
        # 向量化: 结果选择 MUX 级联
        is_overflow_16 = is_overflow.expand_as(result)   # [..., 16]
        result = self.vec_overflow_mux(is_overflow_16, inf_val, result)  # [..., 16]

        is_underflow_16 = is_underflow.expand_as(result)  # [..., 16]
        result = self.vec_underflow_mux(is_underflow_16, zero_val, result)  # [..., 16]

        is_inf_16 = is_inf.expand_as(result)  # [..., 16]
        result = self.vec_inf_mux(is_inf_16, inf_val, result)  # [..., 16]

        is_nan_16 = is_nan.expand_as(result)  # [..., 16]
        result = self.vec_nan_mux(is_nan_16, nan_val, result)  # [..., 16]
    ...
    def reset(self):
        ...
        self.overflow_cmp.reset()
        self.underflow_cmp.reset()
        self.nan_exp_and.reset()
        self.nan_mant_or.reset()
        ...
        self.exp_inc.reset()
        # 向量化门电路
        self.vec_overflow_mux.reset()
        self.vec_underflow_mux.reset()
        self.vec_nan_mux.reset()
        self.vec_inf_mux.reset()
        self.vec_round_exp_mux.reset()
        self.vec_mant_clear_and.reset()
